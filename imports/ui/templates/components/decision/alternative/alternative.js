...
function _generateAlternativeFeed () {
  Session.set('alternativeFeed', Contracts.find({ $or: [{ collectiveId: Meteor.settings.public.Collective._id, stage: { $not: 'DRAFT' } }, { owner: Meteor.user()._id, _id: { $not: Session.get('contract')._id } }] }).fetch());
  if (Session.get('alternativeFeed').length > 0) {
    Session.set('emptyAlternatives', false);
  } else {
