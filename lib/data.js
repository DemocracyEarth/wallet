...
saveDescription = function (newHTML) {
  if (newHTML != getContract(Session.get('contractId')).description) {
    Meteor.call("updateContractField", Session.get('contractId'), "description", newHTML);
    //Contracts.update( { _id: Session.get('contractId') }, { $set: { description: newHTML} },  { reactive: false });
