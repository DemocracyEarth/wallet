...
const _scan = async () => {
  ...
  for (const dao of collectives) {
    ...
    if (!summoning) {
      ...
      console.log(editedTemplate.profile);
      ...
      editedTemplate.profile.bio = (dao.profile.bio === '{{bio}}') ? TAPi18n.__('scan-boilerplate-bio').replace('{{DAOType}}', TAPi18n.__(`dao-type-${editedTemplate.kind.toLowerCase()}`)).replace('{{address}}', dao.status.publicAddress.toLowerCase()) : dao.profile.bio;
