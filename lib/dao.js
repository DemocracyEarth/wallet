...
import { getEvents, sync, getBlockHeight, getLastTimestamp, hasDAO, getState, setToken } from '/lib/web3';
...
const _insertToken = async (token) => {
  return await Tokens.insert(token, (error, result) => {
    if (error) {
      log('[dao WARNING] Insert Error.');
      log(error);
    }
    if (result) {
      log('[dao] Token successfully inserted');
    }
    return result;
  });
};
...
const _insertCollective = async (settings) => {
  return await Collectives.insert(settings, (error, result) => {
    if (error) {
      log('[dao WARNING] Insert Error.');
      log(error);
    }
...
const _install = async (publicAddress, settings) => {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      await _insertCollective(finalSettings);
      resolve();
    } catch (error) {
      reject(error);
    }
...
const _insert = async () => {
  ...
  for (const dao of presetDAOs) {
    ...
    if (collective.length === 0) {
      await _install(dao.profile.blockchain.publicAddress, dao);
    } else {
...
const _scan = async () => {
  ...
  for (const dao of collectives) {
    ...
    if (!summoning) {
      ...
      const tokenAddress = _.findWhere(_.findWhere(editedTemplate.profile.blockchain.smartContracts, { label: 'DAO' }).parameter, { name: 'approvedToken' }).value;
      let coin = Tokens.findOne({ contractAddress: RegExp(['^', tokenAddress, '$'].join(''), 'i') });
      if (!coin) {
        await setToken(tokenAddress);
        let noToken = true;
        while (noToken) {
          coin = Tokens.findOne({ contractAddress: RegExp(['^', tokenAddress, '$'].join(''), 'i') });
          if (coin) {
            noToken = false;
            break;
          } else {
      ...
      const domain = `/dao/${dao.status.publicAddress.toLowerCase()}`;
      ...
      editedTemplate.uri = editedTemplate.name.replace(/[^a-zA-Z0-9]/g, '').replace(/\s+/g, '');
      ...
      editedTemplate.profile.logo = (!dao.profile.logo || dao.profile.logo === '{{logo}}') ? '/images/ethereum-landing.png' : dao.profile.logo;
...
export const insertToken = _insertToken;
