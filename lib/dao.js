  ...
  for (const dao of collectives) {
    ...
    if (!summoning) {
      ...
      switch (summoning.kind) {
        case 'MOLOCH':
        default:
          console.log('SUMMONING PARSE:');
          for (const template of DAOTemplates) {
            console.log(template);
            if (template.kind === 'MOLOCH') {
              editedTemplate = template;

              editedTemplate.name = TAPi18n.__('scan-boilerplate-name').replace('{{init}}', dao.status.publicAddress.slice(2, 6)).replace('{{end}}', dao.status.publicAddress.slice(38, 42));
