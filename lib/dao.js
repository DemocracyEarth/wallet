...
*/
const _getDAOTransactions = async () => {
      ...
      if (collectives[i].profile.blockchain.contracts && collectives[i].profile.blockchain.contracts.length > 0) {
        for (let k = 0; k < collectives[i].profile.blockchain.contracts.length; k += 1) {
          if (collectives[i].profile.blockchain.contracts[k].abi) {
            console.log(`abi: ${JSON.stringify(collectives[i].profile.blockchain.contracts[k].abi)}`);
            daoContract = await getContract(collectives[i].profile.blockchain.contracts[k].publicAddress, collectives[i].profile.blockchain.contracts[k].abi).then((res) => {
              return res;
            });
          }
        }
