...
*/
const _scan = (publicAddress) => {
  console.log(`[dao] Scanning public address: ${publicAddress}....`);
  ...
  const parameters = query({ view: 'addressDao', publicAddress });
  const collective = Collectives.findOne(parameters.find, parameters.options);

  if (!collective) {
    const collectiveId = Collectives.insert(temporary, (error, result) => {
      if (error) {
        log('[dao WARNING] Insert Error.');
        log(error);
      }
...
const _refresh = async () => {
  console.log('[dao] Refreshing DAO data....');
  await _refreshDAOs();
...
const _setupDAOs = async () => {
  await _insertDAOs().then(async (res) => {
    if (res) {
      console.log('[dao] Initial DAO setup....');
      await _refresh();
      finishedSetup = true;
    }
  });
