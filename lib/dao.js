...
let _openCircuit = true;
let _finishedSetup = false;
...
const _insert = async () => {
  ...
  initialDAOCount = presetDAOs.length;
        ...
        default:
          for (const template of DAOTemplates) {
            if (template.kind === 'MOLOCH') {
              ...
              // persist Guild address
              log(`[dao] Guild of DAO found on ${daoState.guildBank.toLowerCase()}`);
              for (const contract of editedTemplate.profile.blockchain.smartContracts) {
                if (contract.label === 'GuildBank' && daoState.guildBank) {
              ...
              editedTemplate.name = (dao.name === '{{name}}') ? TAPi18n.__('scan-boilerplate-name').replace('{{init}}', dao.status.publicAddress.slice(2, 6).toUpperCase()).replace('{{end}}', dao.status.publicAddress.slice(38, 42).toUpperCase()) : dao.name;
              editedTemplate.profile.blockchain.publicAddress = dao.status.publicAddress.toLowerCase();
              editedTemplate.domain = (dao.domain === '{{domain}}') ? domain : dao.domain;
              editedTemplate.profile.website = (dao.profile.website === '{{website}}') ? domain : dao.profile.website;
              editedTemplate.profile.logo = (dao.profile.logo === '{{logo}}') ? `${Meteor.settings.public.app.url.replace(/\/$/, '')}/images/ethereum-landing.png` : dao.profile.logo;
              editedTemplate.profile.bio = (dao.profile.bio === '{{bio}}') ? TAPi18n.__('scan-boilerplate-bio').replace('{{DAOType}}', TAPi18n.__(`dao-type-${template.kind.toLowerCase()}`)).replace('{{address}}', dao.status.publicAddress.toLowerCase()) : dao.profile.bio;
