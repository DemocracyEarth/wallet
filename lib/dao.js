...
const _computeDAOstats = () => {
  ...
  for (let i = 0; i < collectives.length; i += 1) {
    ...
    for (let k = 0; k < collectives[i].profile.menu.length; k += 1) {
      if (collectives[i].profile.menu[k].url && collectives[i].profile.menu[k].url.match(QUERY_KEYWORD)) {
        url = collectives[i].profile.menu[k].url;
        dataKeyworkd = url.replace(QUERY_KEYWORD, '').toUpperCase();
        count = Contracts.find({ period: dataKeyworkd }).count();
        log(`[dao] This DAO has ${count} proposals labeled as '${dataKeyworkd.toLowerCase()}'`);
