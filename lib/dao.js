...
import { getEvents, sync, getBlockHeight, getLastTimestamp, hasDAO, getState, getToken } from '/lib/web3';
...
const _scan = async () => {
  ...
  for (const dao of collectives) {
    ...
    if (!summoning) {
      ...
      let coin = Tokens.findOne({ contractAddress: RegExp(['^', ticker, '$'].join(''), 'i') });
      if (!coin) {
        coin = Tokens.findOne({ _id: await getToken(ticker) });
      }
