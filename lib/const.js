...
*/
const FILTER_ON = false;
const FILTER_CODE = '[web3]';
const _log = (message) => {
  if (Meteor.isServer) {
    if (FILTER_ON && (typeof message === 'string' && message.match(FILTER_CODE) !== null)) {
      console.log(message);
    } else if (!FILTER_ON) {
...
const _gui = {
  ...
  ITEMS_PER_PAGE: 20, // max items on lazy load
  ...
  MOLOCH_DAPP: true,
  LIMIT_TRANSACTIONS_PER_LEDGER: 12,
};
