...
_views.singleDelegationContract = (terms) => {
  if (Meteor.user()) {
    return {
      find: { $and: [{ signatures: { $elemMatch: { username: Meteor.user().username } } }, { signatures: { $elemMatch: { _id: terms.delegateId } } }, { kind: 'DELEGATION' }] },
      options: {},
    };
  }
  return undefined;
