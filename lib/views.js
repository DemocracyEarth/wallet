...
_views.singleDao = (terms) => {
  log(`{ view: 'singleDao', user: ${logUser()}, name: ${terms.name} }`);
  return {
    find: { name: new RegExp(['^', terms.name, '$'].join(''), 'i') },
    options: {},
  };
};
...
_views.addressDao = (terms) => {
  ...
  return {
    find: { $or: [{ 'profile.blockchain.publicAddress': new RegExp(['^', terms.publicAddress, '$'].join(''), 'i') }, { name: new RegExp(['^', terms.publicAddress, '$'].join(''), 'i') }] },
    options: {},
  };
};
