...
_views.latest = (terms) => {
  ...
  if (Meteor.settings.public.app.config.interface.showMainFeedWithReplies) {
    finalQuery = { stage: { $ne: 'DRAFT' }, kind: { $ne: 'DELEGATION' } };
  } else {
    finalQuery = { stage: { $ne: 'DRAFT' }, kind: { $ne: 'DELEGATION' }, replyId: { $exists: false } };
  }
