...
const _getQuery = (view, terms) => {
  switch (view) {
    case 'token':
      return { stage: { $ne: 'DRAFT' }, 'blockchain.coin.code': terms.token.toUpperCase(), collectiveId: (terms.collectiveId) ? terms.collectiveId : { $exists: true } };
    case 'geo':
    ...
    case 'period':
      return { stage: { $ne: 'DRAFT' }, kind: { $ne: 'DELEGATION' }, period: terms.period.toUpperCase(), pollId: { $exists: false }, collectiveId: (terms.collectiveId) ? terms.collectiveId : { $exists: true } };
    case 'latest':
    default:
      return { stage: { $ne: 'DRAFT' }, kind: { $ne: 'DELEGATION' }, collectiveId: (terms.collectiveId) ? terms.collectiveId : { $exists: true }, period: { $ne: 'RAGEQUIT' } };
  }
};
...
_views.latest = (terms) => {
  ...
  if (Meteor.settings.public.app.config.interface && Meteor.settings.public.app.config.interface.mainFeed && Meteor.settings.public.app.config.interface.mainFeed.onlyParentPosts) {
    query = { stage: { $ne: 'DRAFT' }, kind: { $ne: 'DELEGATION' }, pollId: { $exists: false }, replyId: { $exists: false }, period: { $ne: 'RAGEQUIT' } };
  } else {
    query = { stage: { $ne: 'DRAFT' }, kind: { $ne: 'DELEGATION' }, pollId: { $exists: false }, period: { $ne: 'RAGEQUIT' } };
  }
  return {
    find: query,
...
*/
_views.search = (terms) => {
  log(`{ view: 'search', user: ${logUser()}, search: '${terms.search}' }`);
  const query = { stage: { $ne: 'DRAFT' }, kind: { $ne: 'DELEGATION' }, title: { $regex: `.*${decodeURI(terms.search)}.*`, $options: 'i' }, pollId: { $exists: false } };
  return {
    find: query,
    options: { sort: { timestamp: -1 }, limit: terms.limit, skip: terms.skip },
  };
...
_views.transactionsPeer = (terms) => {
  let contractId;
  if (!terms.userId && terms.username) {
    contractId = Meteor.users.findOne({ username: terms.username.toLowerCase() })._id;
  } else {
    contractId = terms.userId;
  }
...
_views.dateRange = (terms) => {
  ...
  return {
    find: { timestamp: { $gte: from, $lt: until }, stage: { $ne: 'DRAFT' }, kind: { $ne: 'DELEGATION' }, pollId: { $exists: false }, replyId: { $exists: false }, period: { $ne: 'RAGEQUIT' } },
    options: { sort: { timestamp: -1 }, limit: terms.limit, skip: terms.skip },
  };
};
...
/**
* @summary queries collectives for a single dao
* @param {object} terms filters and limits
* @return {object} query to use on collection
*/
_views.singleDao = (terms) => {
  log(`{ view: 'singleDao', user: ${logUser()}, name: ${terms.uri} }`);
  return {
    find: { uri: new RegExp(['^', terms.uri, '$'].join(''), 'i') },
    options: {},
  };
};

/**
* @summary queries a DAO by its address
* @param {object} terms filters and limits
* @return {object} query to use on collection
*/
_views.addressDao = (terms) => {
  log(`{ view: 'addressDao', user: ${logUser()}, address: '${terms.publicAddress}' }`);
  return {
    find: { $or: [{ 'profile.blockchain.publicAddress': new RegExp(['^', terms.publicAddress, '$'].join(''), 'i') }, { uri: new RegExp(['^', terms.publicAddress, '$'].join(''), 'i') }] },
    options: {},
  };
};

/**
* @summary specific feed to a dao
* @param {object} terms filters and limits
* @return {object} query to use on collection
*/
_views.dao = (terms) => {
  log(`{ view: 'dao', user: ${logUser()} }`);

  const collective = Collectives.findOne({ name: terms.name });

  return {
    find: { collectiveId: collective._id, stage: { $ne: 'DRAFT' }, kind: { $ne: 'DELEGATION' }, pollId: { $exists: false }, replyId: { $exists: false }, period: (terms.period === '' || !terms.period) ? { $ne: 'RAGEQUIT' } : terms.period.toUpperCase() },
    options: { sort: { timestamp: -1 }, limit: terms.limit, skip: terms.skip },
  };
};

/**
* @summary latest transactions in a dao
* @param {object} terms filters and limits
* @return {object} query to use on collection
*/
_views.transactionsDao = (terms) => {
  const collective = Collectives.findOne({ name: terms.name });

  log(`{ view: 'transactionsDao', user: ${logUser()}, collectiveId: '${collective._id}' }`);

  return {
    find: { collectiveId: collective._id },
    options: { limit: 25, sort: terms.sort },
  };
};

/**
* @summary returns transactions based on search terms
* @param {object} terms filters and limits
* @return {object} query to use on collection
*/
_views.transactionsSearch = (terms) => {
  log(`{ view: 'transactionsSearch', user: ${logUser()}, search: '${terms.search}' }`);
  const contracts = Contracts.find({ stage: { $ne: 'DRAFT' }, kind: { $ne: 'DELEGATION' }, title: { $regex: `.*${decodeURI(terms.search)}.*`, $options: 'i' }, pollId: { $exists: false } }).fetch();
  if (contracts.length > 0) {
    const contractIds = _.uniq(_.pluck(contracts, '_id'));
    const query = [];
    for (const id of contractIds) {
...
/**
* @summary returns a list of collectives listed by id
* @param {object} terms filters and limits
* @return {object} query to use on collection
*/
_views.collectiveList = (terms) => {
  log(`{ view: 'collectiveList', user: ${logUser()}, list: '${terms.list}' }`);
  const query = [];
