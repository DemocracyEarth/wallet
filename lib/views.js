...
_views.linkedFeed = (terms) => {
  ...
  const finalQuery = Object.assign(_getQuery(terms.subview, terms), { createdAt: { $lt: new Date(terms.lastItem) } });
  return {
    find: finalQuery,
    options: { sort: { createdAt: -1 }, limit: terms.limit, skip: 0 },
  };
};
...
_views.peer = (terms) => {
  ...
  return {
    find: { stage: { $ne: 'DRAFT' }, kind: !terms.kind ? 'VOTE' : terms.kind, signatures: { $elemMatch: _getSignature(terms) } },
...
_views.periodVotes = (terms) => {
  ...
  // get contract referred for view according to period.
  const contracts = Contracts.find({ period: terms.period });

  if (contracts.length > 0) {
    const thread = [];
    for (let k = 0; k < contracts.length; k += 1) {
      thread.push(_getThread(contracts[k]._id));
    }
    ...
    const query = [];
    for (let i = 0; i < thread.length; i += 1) {
      const entities = [{ 'output.entityId': thread[i] }, { 'input.entityId': thread[i] }];
      query.push({ $or: [{ contractId: thread[i] }, { $or: entities }] });
    }

    log(`{ view: 'periodVotes', user: ${logUser()}, contractId: '${contract._id}', threadLength: ${thread.length} }`);
  ...
  log(`{ view: 'periodVotes', user: ${logUser()}, threadLength: 0 }`);
  return {
    find: {},
    options: { limit: 1 },
  };
...
_views.token = (terms) => {
  ...
  if (terms.token) {
    return {
      ...
      options: { sort: terms.sort, limit: terms.limit, skip: terms.skip, period: terms.period },
    };
  }
  return {
    find: {},
