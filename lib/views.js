...
const _mapTree = (root) => {
  ...
  for (let j = 0; j < list.length; j += 1) {
    if (list[j] !== root._id) {
      console.log(`sublist for: ${list[j]}`);
...
*/
_views.post = (terms) => {
  log(`{ view: 'post', user: ${logUser()}, keyword: '${terms.keyword}' }`);
  if (terms.keyword) {
    return {
...
_views.thread = (terms) => {
  ...
  let branches = _mapTree(rootContract);
  branches = _.uniq(_thread);
  console.log(`thread: ${JSON.stringify(_thread)}`);
  const leaf = [];
  for (let i = 0; i < branches.length; i += 1) {
    leaf.push({ _id: branches[i] }, { replyId: branches[i] });
    console.log(`branchId: ${branches[i]}`);
  }
  log(`{ view: 'thread', user: ${logUser()}, contractId: '${contract._id}', rootId: '${rootContract._id}', posts: ${branches.length} }`);
