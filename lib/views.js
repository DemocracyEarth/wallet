...
_views.periodVotes = (terms) => {
  ...
  if (contracts.length > 0) {
    ...
    for (let k = 0; k < contracts.length; k += 1) {
      ...
      if (contracts[k].poll.length > 0) {
        thread.push(..._.pluck(contracts[k].poll, 'contractId'));
      }
    }
    ...
    log(`{ view: 'periodVotes', user: ${logUser()}, period: '${terms.period}', threadLength: ${thread.length} }`);
