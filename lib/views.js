...
_views.search = (terms) => {
  ...
  const query = { stage: { $ne: 'DRAFT' }, kind: { $ne: 'DELEGATION' }, title: { $regex: `.*${decodeURI(terms.search)}.*` }, pollId: { $exists: false } };
  return {
    find: query,
    options: { sort: { timestamp: -1 }, limit: terms.limit, skip: terms.skip },
  };
...
_views.transactionsSearch = (terms) => {
  ...
  const contracts = Contracts.find({ stage: { $ne: 'DRAFT' }, kind: { $ne: 'DELEGATION' }, title: { $regex: `.*${decodeURI(terms.search)}.*` }, pollId: { $exists: false } }).fetch();
