...
_views.linkedFeed = (terms) => {
  ...
  return {
    find: { stage: { $ne: 'DRAFT' }, kind: { $ne: 'DELEGATION' }, createdAt: { $lt: new Date(terms.lastItem) } },
...
_views.peer = (terms) => {
  ...
  return {
    find: { stage: { $ne: 'DRAFT' }, kind, signatures: { $elemMatch: signature } },
...
_views.transactionsGeo = (terms) => {
  log(`{ view: 'transactionsGeo', user: ${logUser()}, geo: '${terms.geo.toUpperCase()}' }`);
  return {
    find: { 'blockchain.coin.code': terms.token.toUpperCase() },
    options: { limit: 25, sort: terms.sort },
  };
