...
const _mapTree = (root) => {
  ...
  let sublist = [];
  list.push(root._id);
  const query = { $or: [{ _id: root._id }, { replyId: root._id }] };
...
_views.linkedFeed = (terms) => {
  log(`{ view: 'linkedFeed', user: ${logUser()}, terms: ${JSON.stringify(terms)} }`);
  return {
    find: { collectiveId: Meteor.settings.public.Collective._id, stage: { $ne: 'DRAFT' }, kind: { $ne: 'DELEGATION' }, createdAt: { $lt: new Date(terms.lastItem) } },
...
_views.post = (terms) => {
  ...
  if (terms.keyword) {
    ...
    return {
      find: { collectiveId: Meteor.settings.public.Collective._id, stage: { $ne: 'DRAFT' }, url: terms.url },
      options: { sort: { lastUpdate: -1 } },
    };
  }
  return {
...
_views.token = (terms) => {
  ...
  if (terms.token) {
    return {
      find: { stage: { $ne: 'DRAFT' }, 'blockchain.coin.code': terms.token.toUpperCase() },
      options: { sort: terms.sort, limit: terms.limit, skip: terms.skip },
    };
  }
  return {
    find: {},
...
_views.geo = (terms) => {
  ...
  if (terms.country) {
    return {
      find: { stage: { $ne: 'DRAFT' }, geo: terms.country.toUpperCase() },
      options: { sort: { lastUpdate: -1 }, limit: terms.limit, skip: terms.skip },
    };
  }
  return {
