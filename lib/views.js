...
/**
* @summary transactions pending on a specific contract from a user
* @param {object} terms filters and limits
* @return {object} query to use on collection
*/
_views.userPendings = (terms) => {
  let contractId;
  if (!terms.userId && terms.username) {
    contractId = Meteor.users.findOne({ username: terms.username })._id;
  } else {
  ...
  log(`{ view: 'userPendings', user: ${logUser()}, contractId: '${contractId}' }`);
  if (contractId) {
    return {
      find: { status: 'PENDING', $or: [{ 'output.entityId': contractId }, { 'input.entityId': contractId }, { 'output.delegateId': contractId }, { 'input.delegateId': contractId }] },
      options: { sort: terms.sort },
    };
  }
  return {
