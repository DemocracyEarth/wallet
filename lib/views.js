...
_views.dao = (terms) => {
  ...
  const collective = Collectives.findOne({ 'profile.blockchain.coin.code': terms['profile.blockchain.coin.code'] });

  return {
    find: { collectiveId: collective._id, stage: { $ne: 'DRAFT' }, kind: { $ne: 'DELEGATION' }, pollId: { $exists: false }, replyId: { $exists: false }, period: { $ne: 'RAGEQUIT' } },
    options: { sort: { timestamp: -1 }, limit: terms.limit, skip: terms.skip },
  };
};
...
_views.transactionsDao = (terms) => {
  ...
  return {
    find: { 'blockchain.coin.code': terms['profile.blockchain.coin.code'] },
    options: { limit: 25, sort: terms.sort },
  };
};
