...
_views.peer = (terms) => {
  ...
  if (terms.username && !terms.userId) {
    userId = Meteor.users.findOne({ username: terms.username })._id;
  } else {
    userId = terms.userId;
  }
  return {
    find: { collectiveId: Meteor.settings.public.Collective._id, stage: { $ne: 'DRAFT' }, signatures: { $elemMatch: { _id: userId } } },
    options: { sort: { lastUpdate: -1 }, limit: terms.limit, skip: terms.skip },
  };
};
