...
_views.bothDelegationContracts = (terms) => {
  if (Meteor.user()) {
    ...
    if (Contracts.find(query).fetch().length === 0) {
      _log(`{ view: 'bothDelegationContracts', user: ${Meteor.user().username}, delegateId: ${terms.delegateId}, `);
      ...
      _log(`log: 'Delegation contract created from ${Meteor.user().username} to ${delegateName}', `);
      ...
      _log(`log: 'Delegation contract created from ${delegateName} to ${Meteor.user().username}' }`);
    }
    return {
      find: query,
      options: {},
