...
/**
* @summary get email from replyEmailWhitelist
* @param {string} ballotId
* @return {string} email associated with ballotId
*/
function _getWhitelistReplyEmail(ballotId) {
  const target = replyEmailWhitelist.find(i => (i.ballotId === ballotId));
  if (target) {
    return target.email;
  }
  return undefined;
...
export const getWhitelistReplyEmail = _getWhitelistReplyEmail;
