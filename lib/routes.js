...
Router.route('/tag/:tag', {
  ...
  data: function () {
    console.log('[tag] data: ' + this.params.query);
  }
});
...
let _buildTitle = (params) => {
  ...
  if (typeof params == 'string') {
    ...
    for (key in params) {
      ...
      switch (key) {
        case 'tag':
          return TAPi18n.__('tagged') + ' <strong><em>' + Tags.findOne({ keyword: params[key] }).text + '</em></strong>';
        case 'hash':
        case 'query':
          break;
        default:
...
let _loadContract = (view, id) => {
  ...
  if (contract != undefined) {
    Session.set('alreadyVoted', false); //NOTE: verify this
    Session.set('contract', contract);
  ...
  if (Session.get('sidebarMenuSelectedId')) {
    ...
    for (item in menu) {
      if (menu[item].selected == true) {
        Session.set('sidebarMenuSelectedId', menu[item].id);
        return menu[item].feed;
      }
    }
  }
