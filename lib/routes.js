...
Router.route('/:feed', {
  ...
  onBeforeAction: function () {
    ...
    setNavigationBar(_buildTitle(this.params.query));
...
Router.route('/delegation/:contract', {
  ...
  onBeforeAction: function() {
    ...
    Session.set("voteKeyword", this.params.contract);
    Session.set('stage', contract.stage);
    setNavigationBar(TAPi18n.__('navbar-delegation'));
    Modules.client.setSidebarMenu('');
    setMainSessionVars();
    this.next();
  },
...
function setMainSessionVars (query) {
  ...
  //general NOTE: verify this on different contexts just in case.
