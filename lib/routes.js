...
Router.route('/:feed', {
  ...
  template: 'home',
  waitOn: function () {
    switch(this.params.feed) {
      case 'filter':
        switch(this.params.query.kind) {
          case 'membership':
          case 'delegation':
            Meteor.subscribe('contracts', { collectiveId: Meteor.settings.public.Collective._id, kind: this.params.query.kind.toUpperCase() });
            break;
          case 'draft':
            Meteor.subscribe('contracts', { collectiveId: Meteor.settings.public.Collective._id, stage: 'DRAFT' });
            break;
          default:
            break;
        }
      ...
      case 'filter':
        switch(this.params.query.kind) {
          ...
          case 'delegation':
            ...
            break;
          case 'draft':
            Session.set('feed', Contracts.find({ stage: 'DRAFT' }, {sort: {timestamp: -1}} ).fetch());
            break;
        }
      default:
        break;
