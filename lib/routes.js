...
Router.route('/', {
  ...
  onBeforeAction: function () {
    ...
    if (Meteor.settings.public.Collective) {
      ...
      Session.set('feed', Contracts.find({ collectiveId: Session.get('collectiveId')}, {sort: {timestamp: -1}} ).fetch());
    };
    //Config Menu
    Session.set('inboxMenu', Modules.client.setMenu('/'));
    this.next();
  }
});
...
Router.route('/:feed', {
  ...
  onBeforeAction: function () {
    ...
    if (this.params.query.kind) {
      Session.set('feed', Contracts.find({ kind: this.params.query.kind.toUpperCase() }, {sort: {timestamp: -1}} ).fetch());
      Session.set('inboxMenu', Modules.client.setMenu('/' + this.params.feed + '?kind=' + this.params.query.kind));
    } else if (this.params.query.stage) {
      Session.set('feed', Contracts.find({ stage: this.params.query.stage.toUpperCase() }, {sort: {timestamp: -1}} ).fetch());
      Session.set('inboxMenu', Modules.client.setMenu('/' + this.params.feed + '?stage=' + this.params.query.stage));
    }
