...
const _persist = (memory, change) => {
  let found = false;
  if (newSettings.profile.wallet && newSettings.profile.wallet.ledger && newSettings.profile.wallet.ledger.length > 0) {
    console.log(`newSettings.profile.wallet.ledger:`);
    console.log(newSettings.profile.wallet.ledger);

    change = newSettings.profile.wallet.ledger
    memory = voter[0].profile.wallet.ledger
    ...
    if (voter[0].profile.wallet && voter[0].profile.wallet.ledger && voter[0].profile.wallet.ledger.length > 0) {
      for (let i = 0; i < memory.length; i += 1) {
        if (memory[i].publicAddress === change[0].publicAddress) {
          memory[i] = change[0];
          break;
        }
      }
...
const _migrateAddress = (address, settings) => {
  ...
  if (voter.length === 0) {
    ...
    // newSettings.profile.wallet.address = _.uniq(voter[0].profile.wallet.address.push(...newSettings.profile.wallet.address));
    // newSettings.profile.wallet.reserves = _.uniq(voter[0].profile.wallet.reserves.push(...newSettings.profile.wallet.reserves));
    ...
    if (newSettings.profile.wallet && newSettings.profile.wallet.reserves && newSettings.profile.wallet.reserves.length > 0) {
      console.log(`newSettings.profile.wallet.reserves:`);
      console.log(newSettings.profile.wallet.reserves);
      found = false;
    ...
    // newSettings.profile.wallet.ledger = _.uniq(voter[0].profile.wallet.ledger.push(...newSettings.profile.wallet.ledger));
...
const _parseContent = (title, elements) => {
  ...
  for (let i = 0; i < keys.length; i += 1) {
    ...
    if (match && match.length > 0) {
      newTitle = newTitle.replace(`{{${keys[i]}}}`, elements[keys[i]].toString().replace(/'/g, '&apos;'));
    }
