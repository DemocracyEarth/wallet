...
const _calculateReserves = (collectives, reserves, ledger) => {
  ...
  for (const collectiveId of collectives) {
    const transactions = _.sortBy(_.where(ledger, { collectiveId }), 'timestamp');
    const finalReserve = _.findWhere(reserves, { collectiveId });
    const tally = _.reduce(transactions, (memo, num) => { return memo.value + num.value; });
...
const _migrateAddress = (address, settings) => {
  ...
  if (voter.length === 0) {
    ...
    log(`[web3] Updateing user with new settings.. ${address.toLowerCase()}`);
    const user = voter[0];
    ...
    // final profile
    if (address === '0x865c2f85c9fea1c6ac7f53de07554d68cb92ed88') {
