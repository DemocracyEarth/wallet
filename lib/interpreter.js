...
const _getPeriod = (timestamp, closing, state, index) => {
  const now = parseInt(timestamp * 1000, 10);
  const moment = parseInt(closing - now, 10);
  const votingAndProcessed = parseInt(state.periodDuration + state.votingPeriodLength + state.gracePeriodLength, 10);
  let finality;

  log(`[DATE CALCULATION] moment: ${moment}`);
  log(`[DATE CALCULATION] now: ${now}`);
  log(`[DATE CALCULATION] closing: ${closing}`);

  log(`[DATE CALCULATION] state.processed: ${state.processed}`);
  log(`[DATE CALCULATION] state.proposalQueue[index] ${JSON.stringify(state.proposalQueue[index])}`);

  if (state.processed) {
    finality = 'PROCESSED';
  }
  if (state.processed && (moment <= votingAndProcessed)) {
    finality = 'GRACE';
  }
  if (state.processed && (moment <= (state.periodDuration + state.votingPeriodLength))) {
    finality = 'VOTING';
  }
  if (state.processed && (moment <= state.periodDuration)) {
    finality = 'QUEUE';
  }
  if (moment <= 0) {
    finality = 'COMPLETE';
...
const _getClosingObject = (state, height, blockTimestamp, index) => {
  ...
  const closingCalendar = new Date(parseInt(blockTimestamp + votingTotalLength, 10));
