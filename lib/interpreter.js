...
/**
* @summary emulates moloch dao clock
* @param {number} summoningTime of the dao
* @param {number} periodDuration for processing times
*/
...
* @param {string} index from proposal queue
*/
const _getFinality = (state, height, blockTimestamp, index) => {
  ...
  if (transcurredPeriods < abortWindow) {
    period = 'QUEUE';
  } else if (transcurredPeriods < (abortWindow + votingPeriodLength)) {
    period = 'VOTING';
  } else if (transcurredPeriods < (abortWindow + votingPeriodLength + gracePeriodLength)) {
    period = 'GRACE';
  } else if (state.proposalQueue[index].processed) {
    period = 'COMPLETE';
  }
