...
import { getClosingObject } from './interpreter';
...
* @param {object} parameter settings required to parse this contract
* @param {string} collectiveId this is being subscribed to
*/
const _mirrorContract = (event, map, parameter, collectiveId) => {
    ...
    const blockDate = web3.eth.getBlock(event.blockNumber, (err, res) => {
      ...
      if (!err) {
        ...
        const contract = {
          ...
          pollId: '',
          collectiveId,
          closing: getClosingObject(parameter, event.blockNumber),
        };
...
const _writeEvents = (event, smartContract, collectiveId) => {
  ...
  const parameter = smartContract.parameter;
          ...
          switch (map[k].collectionType) {
            ...
            case 'Contract':
              if (event[i].event === 'SubmitProposal') {
                _mirrorContract(event[i], map[k], parameter, collectiveId);
              }
