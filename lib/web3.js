...
const _generateDataset = async (feed, coin) => {
  ...
  for (let i = 0; i < feed.length; i += 1) {
    ...
    dataset.push({
      label: new Date(block.timestamp * 1000).getTime(),
...
const _buildChart = async (coin, publicAddress) => {
  console.log(`defaults.START_BLOCK: ${defaults.START_BLOCK}`);
  ...
  const dataset =
    _.reject(
      _.sortBy(
        Object.assign(
          await _generateDataset(income, coin),
          _.map(await _generateDataset(outcome, coin), (item) => { const newItem = item; newItem.data *= -1; return newItem; })
        ), 'label'),
      (item) => { return (item.data === 0); });
