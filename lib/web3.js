...
import { migrateAddress, getContractObject, getTransactionObject, parseContent } from '/lib/interpreter';
...
const _setContract = (keyword, contractObject) => {
  ...
  if (dbContract) {
    log('[web3] Updating existing contract...', dbContract._id);
    log(dbContract._id, dbContract._id);
    log('blockchain object:', dbContract._id);
    log(contractObject.blockchain, dbContract._id);
    log(contractObject.collectiveId, dbContract._id);
    

    Contracts.update({ _id: dbContract._id }, { $set: { blockchain: contractObject.blockchain } }, (err, res) => {
      log('UPDATED', dbContract._id);
      log(res, dbContract._id);
      log(Contracts.find({ _id: dbContract._id }).fetch(), dbContract._id);

      if (err) {
        log('ERROR', dbContract._id);
        log(err, dbContract._id);
      }
      return res;
    });
    log('[web3] RETURN', dbContract._id);
    return dbContract._id;
  }
  log('[web3] Inserting new contract..', dbContract._id);
...
*/
const _mirrorContract = (event, map, collectiveId) => {
  ...
  if (user) {
    const blockDate = web3.eth.getBlock(event.blockNumber, (err, res) => {
      if (!err) {
        ...
        const contract = {
          title: parseContent(TAPi18n.__(map.contract.title), event.returnValues),
          keyword: `${event.transactionHash}`,
          url: `${_timestampToDate(block.timestamp)}${event.transactionHash}`,
          date: new Date(block.timestamp * 1000),
          publicAddress: event.address,
...
* @param {string} collectiveId this is being subscribed to
*/
const _writeEvents = (event, smartContract, collectiveId) => {
          ...
          switch (map[k].collectionType) {
            ...
            case 'Contract':
              if (event[i].event === 'SubmitProposal') {
                _mirrorContract(event[i], map[k], collectiveId);
              }
      ...
      await new web3.eth.Contract(abi, smartContract.publicAddress).getPastEvents('allEvents', {
        ...
        if (error) {
          ...
          if (res.length > 0 && smartContract.map && smartContract.map.length > 0) {
            _writeEvents(res, smartContract, collectiveId);
          }
        }
        return res;
      }).then((res) => {
