        ...
        case 'Proposal':
          ...
          for (let k = 0; k < length; k += 1) {
            await dao.methods[`${smartContract.parameter[i].name}`](k).call({}, (err, res) => {
              ...
              log(`[web3] Got struct proposal titled: "${res.details.substring(0, 100)}..."`);
              struct.push(res);
              return res;
            });
          }
    ...
    for (let k = 0; k < collectives[i].profile.guild.length; k += 1) {
      switch (collectives[i].profile.guild[k].name) {
        case 'guild-total-shares':
          finalGuild[k].value = await _callMethod(collectives[i], 'totalShares'); // _getTokenBalance({ period: 'PASSED', 'decision.requestToken': ticker }, { period: 'RAGEQUIT', 'wallet.currency': ticker }, 'request', 'sharesToBurn');
        ...
        case 'guild-total-weth':
          ...
          finalGuild[k].value = _getTokenBalance({ period: 'PASSED', 'decision.tributeToken': ticker }, { period: 'RAGEQUIT', 'wallet.currency': ticker }, 'tribute', 'withdrawnTribute');
          log(`[dao] Total shares for this DAO... ${finalGuild[k].value}`);
          Collectives.update({ _id: collectives[i]._id }, { $set: { 'profile.guild': finalGuild } });
          break;
