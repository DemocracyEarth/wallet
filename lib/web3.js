...
const _getShares = (member, ticker) => {
  log(`[web3] _getShares member: ${JSON.stringify(member)}`);
  console.log(member);
  log(`[web3] _getShares ticker: ${ticker}`);
  log(`[web3] member.profile.wallet.reserves: ${member.profile.wallet.reserves.length}`);
  console.log(member.profile.wallet.reserves);
  let shares = 0;
  for (let i = 0; i < member.profile.wallet.reserves.length; i += 1) {
    log(`[web3] member.profile.wallet.reserves[i].token: ${member.profile.wallet.reserves[i].token}`);
