...
* @param {object} mapping how to mp this smart contract on the database
*/
const _getEvents = async (publicAddress, interfaceJSON) => {
  if (_web3()) {
    ...
    if (abi) {
      const contractEvents = await new web3.eth.Contract(abi, publicAddress).getPastEvents('allEvents', {
