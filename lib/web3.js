...
* @param {object} mpa with info how to write these eventos on the blockchain
*/
const _writeEvents = (log, map) => {
...
* @summary show all the transactions for a given public address
* @param {string} publicAddress of a contract.
* @param {string} interfaceJSON the abi of a smart contract
* @param {boolean} doMapping if smart contract info should be mapped to db
*/
const _getEvents = async (publicAddress, interfaceJSON, doMapping) => {
  let eventLog;
  ...
  if (_web3()) {
    ...
    if (abi) {
      await new web3.eth.Contract(abi, publicAddress).getPastEvents('allEvents', {
