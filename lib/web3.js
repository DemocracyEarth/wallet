...
const _getUserAddress = (res, collectiveId, role) => {
  ...
  for (let i = 0; i < addresses.length; i += 1) {
    ...
    console.log(addresses[i].toLowerCase());
      ...
      if (reserves) {
        for (let i = 0; i < reserves.length; i += 1) {
          if (reserves[i].token === defaults.TOKEN && reserves[i].publicAddress === authorUsername) {
            reserves[i].balance += parseInt(new BigNumber(event.returnValues.sharesRequested).toNumber(), 10);
          }
        }
      }
    } else {
      console.log(event.returnValues.sharesRequested);
      console.log(event.returnValues);
      shares = parseInt(new BigNumber(event.returnValues.sharesRequested).toNumber(), 10);
      console.log(shares);
