...
import { migrateAddress, getContractObject } from '/lib/interpreter';
...
const _mirrorContract = (log, map, collectiveId) => {
  ...
  if (user) {
    ...
    console.log(log);
    console.log(map);
    const keyword = contractURI(TAPi18n.__(map.contract.title));
    const draft = {
      createdAt: new Date(),
    };
    const settings = {
      title: TAPi18n.__(map.contract.title),
      keyword,
      url: `${_heightToDate(draft.createdAt)}${keyword}`,
      date: new Date(),
    ...
    // const _getContractObject = (user, title, keyword, url, date, publicAddress, height, calendar, importId, pollChoiceId, pollId) => {
      // title, keyword, url, date, publicAddress, height, calendar, importId, pollChoiceId, pollId
    const contractObject = getContractObject(user, settings);
