...
const _getEventBlockTimestamp = async (blockNumber) => {
  let returnDate = new Date();
  console.log(`[web3] Getting events on block number: ${blockNumber}`);

  await web3.eth.getBlock(blockNumber, (err, res) => {
    if (err) {
      console.log(err);
    }
    returnDate = new Date(res.timestamp * 1000);
    log(`[web3] Timestamp on current block ${blockNumber} is ${returnDate}`);
    return res;
  });
  return returnDate;
};
...
const _writeEvents = async (event, smartContract, state, collectiveId) => {
  ...
  log(`[web3] currentEventBlockTimestamp: ${currentEventBlockTimestamp}`);
  ...
  log(`[web3] lastEventBlockTimestamp: ${lastEventBlockTimestamp}`);
  ...
  if (lastEventBlockTimestamp <= currentEventBlockTimestamp) {
    ...
    for (let i = 0; i < event.length; i += 1) {
      ...
      eventTimestamp = await _getEventBlockTimestamp(event[i].blockNumber);
