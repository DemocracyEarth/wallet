...
const _updateChart = (legacy, update) => {
  const startingPoint = parseInt(legacy.length - 1, 10);
  const finalUpdate = legacy;
  let newY;
  for (let i = 0; i < update.length; i += 1) {
    if (i === 0) {
      newY = parseFloat(legacy[startingPoint].y + update[i].y, 10);
    } else {
      newY = parseFloat(update[i].y + update[i - 1].y, 10);
    }
    finalUpdate.push({
      t: update[i].t,
      y: newY,
      ...
      switch (collectives[i].profile.guild[k].name) {
        ...
        case 'guild-total-assets':
          ...
          if (coin) {
            ...
            // const initialBlock = _.findWhere(collectives[i].profile.chart, { guildLabel: collectives[i].profile.guild[k].name }).lastSyncedBlock;
