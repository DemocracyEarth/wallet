...
const _checkPrecedent = (username, collectiveId) => {
  ...
  log(`[web3] Checking precedent for ${username} on ${collective.profile.blockchain.publicAddress}...`);
  const reserve = _.findWhere(user.wallet.reserves, { publicAddress: collective.profile.blockchain.publicAddress.toLowerCase() });

  if (reserve && reserve.balance) {
    log(`[web3] Current shares: ${reserve.balance}`);
    return reserve.balance;
  }

  log('[web3] No precedent');
  return 0;
