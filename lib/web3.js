...
const _precomputeLastEventIndex = (event, smartContract, state) => {
...
const _writeEvents = (event, smartContract, state, collectiveId) => {
  ...
  let newLastEventIndex = _precomputeLastEventIndex(event, smartContract, state);

  if (lastEventIndex <= newLastEventIndex) {
    const map = smartContract.map;
  ...
  // save the index of last event
  if (lastEventIndex < newLastEventIndex) {
    Collectives.update({ _id: collectiveId }, { $set: { 'profile.lastEventIndex': newLastEventIndex } }, (err, res) => {
      if (err) {
        console.log(err);
      }
      log(`[web3] Updated collective with lastEventIndex ${newLastEventIndex}`);
      return res;
    });
  }
};
