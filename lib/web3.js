    ...
    if (abi) {
      ...
      const contractEvents = new web3.eth.Contract(abi, publicAddress).getPastEvents('allEvents', {
        ...
        if (error) {
          ...
          console.log(`[web3] Log has a length of ${log.length} events.`);
          for (let i = 0; i < log.length; i += 1) {
            console.log(`[web3] event: ${log[i].event}`);
            console.log(`[web3] returnValues: ${JSON.stringify(log[i].returnValues)}`);
          }
        }
        return log;
      });
