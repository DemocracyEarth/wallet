...
const _writeEvents = (event, smartContract, state, collectiveId) => {
  ...
  const precomputedIndex = _precomputeLastEventIndex(event, smartContract, state);
  let processingIndex = 0;

  log(`[web3] Obtaining from DB lastEventIndex: ${lastEventIndex}`);
  log(`[web3] Calculating from blockchain precomputedIndex: ${precomputedIndex}`);

  if (lastEventIndex < precomputedIndex) {
    const map = smartContract.map;

    // TODO: proposalQueue should be obtained from smartContract object
    for (let k = 0; k < state.proposalQueue.length; k += 1) {
      if (state !== null) {
        if (processingIndex >= precomputedIndex) {
