        ...
        web3.eth.getBlock(event.blockNumber, (err, res) => {
          ...
          if (voter) {
            ...
            if (!err) {
              ...
              const voterList = poll.tally.voter;
              ...
              if (!found) {
                console.log(`pushing userId ${userId} and shares: ${shares}`);
                voterList.push({
                  _id: userId,
              ...
              console.log(`voterList FINALE`);
              console.log(voterList);

              poll.tally.lastTransaction = txId;
              poll.tally.voter = voterList;

              console.log(`updateing poll.importId: ${poll.importId}`);
              console.log(poll.tally);

              Contracts.update({ importId: poll.importId }, { $set: { tally: poll.tally } });

              // _setContract(poll.importId, poll);
            }
          } else {
            log(err);
          }
