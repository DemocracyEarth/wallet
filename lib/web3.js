...
const _setContract = (keyword, contractObject) => {
  ...
  if (dbContract) {
    console.log('[web3] Updating existing contract...');
  ...
  console.log('[web3] Inserting new contract...');
...
const _getAddressList = (log, collectiveId) => {
  let settings;
  let membership;
  let authorUsername;
...
const _mirrorContract = (log, map, collectiveId) => {
  ...
  const authorUsername = _getAddressList(log, collectiveId);
  ...
  if (user) {
    const blockDate = web3.eth.getBlock(log.blockNumber, (err, res) => {
      if (!err) {
        const block = res;
        const index = new BigNumber(log.returnValues.proposalIndex).toString();
        const contract = {
          title: parseContent(TAPi18n.__(map.contract.title), log.returnValues),
          keyword: `${log.transactionHash}`,
          url: `${_timestampToDate(block.timestamp)}${log.transactionHash}`,
        ...
        if (map.contract.rules.pollVoting) {
          ...
          for (let k = 0; k < choices.length; k += 1) {
            ...
            choice.keyword = `${log.transactionHash}/${choices[k]}`;
          ...
          console.log(`[web3] Poll added to contract: ${newContractId}`);
        }
      }
    });
  }
...
* @param {string} collectiveId this is being subscribed to
*/
const _mirrorTransaction = (log, map, collectiveId) => {
  console.log(`[web3] Mirroring blockchain event as transaction action with collectiveId: ${collectiveId}...`);
  console.log(log);
  ...
  if (user) {
    ...
    if (contract) {
      let poll;
      switch (log.returnValues.uintVote) {
        case 1: // yes
  ...
  for (let i = 0; i < log.length; i += 1) {
    for (let k = 0; k < map.length; k += 1) {
      if (map[k].eventName === log[i].event) {
        console.log(`[web3] Processing event: ${log[i].event}`);
        ...
        if (map[k].eventName === log[i].event) {
          switch (map[k].collectionType) {
            ...
            case 'Contract':
              ...
              if (log[i].event === 'SubmitVote') {
                _mirrorTransaction(log[i], map[k], collectiveId);
              }
              break;
            default:
          }
