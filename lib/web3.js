...
const _mirrorContractEvent = (event, map, state, collectiveId, block) => {
  ...
  const user = Meteor.users.findOne({ username: event.returnValues.delegateKey.toLowerCase() });
  ...
  if (user && proposal && proposal.applicant) {
    ...
    const contract = {
      ...
      decision: {
        ...
        request: new BigNumber(proposal.request).toString(),
        tribute: new BigNumber(proposal.tribute).toString(),
        ...
        case 'Proposal':
          ...
          for (let k = 0; k <= length; k += 1) {
            await dao.methods[`${smartContract.parameter[i].name}`](k).call({}, (err, res) => {
              ...
              log(`[web3] Got struct data: ${JSON.stringify(res)}`);
              struct.push(res);
              return res;
            });
          }
