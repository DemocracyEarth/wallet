...
const _updateStatus = (collective, stage, increment, message) => {
  ...
  const newStatus = {
    ...
    blockchainSync: (newPercentage > 100) ? 'UPDATED' : stage,
...
*/
const _getState = async (smartContract, collective) => {
  const state = {};
  ...
  if (smartContract.parameter) {
    ...
    for (let i = 0; i < smartContract.parameter.length; i += 1) {
      increment += _calcIncrement(25, smartContract.parameter.length);
      if (collective) {
        _updateStatus(collective, 'SYNCING', increment, TAPi18n.__('synchronizer-proposals'));
      }
  ...
  if (_web3()) {
    for (const template of DAOTemplates) {
      ...
      for (const contract of template.profile.blockchain.smartContracts) {
        log(`[web3] Reading smart contract ${contract.label}...`);
