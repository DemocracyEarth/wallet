...
const _setContract = (importId, contractObject) => {
  ...
  if (dbContract) {
    ...
    log(`[web3] contractObject.timestamp: ${contractObject.timestamp}`);
...
const _mirrorContractEvent = (event, map, state, collectiveId) => {
  ...
  if (user) {
    web3.eth.getBlock(event.blockNumber, (err, res) => {
      console.log(` web3.eth.getBlock web3.eth.getBlock web3.eth.getBlock web3.eth.getBlock web3.eth.getBlock`);
      ...
      if (!err) {
        ...
        let contract;
        log(`[web3] ABOUT TO UPDATE THIS DB IN BLOCK EVENT..`)
        ...
        if (contractDB) {
          ...
          console.log(`calendar------------------------:${contractBlockData.calendar}`);
        ...
        web3.eth.getBlock(event.blockNumber, (err, res) => {
          ...
          if (voter) {
            ...
            if (!err) {
              ...
              log(`[web3] pollChoiceContract.timestamp: ${pollChoiceContract.timestamp}`);
