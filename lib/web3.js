...
const _mirrorUserEvent = (event, map, collectiveId, block) => {
  ...
  console.log(event);
  console.log(map);
  console.log(`event.returnValues.memberAddress ${event.returnValues.memberAddress}`);
  ...
  let reserves;
  console.log(`shares: ${parseInt(parseInt(_checkPrecedent(authorUsername, precedentCache) - new BigNumber(event.returnValues.sharesToBurn).toNumber(), 10), 10)}`);
