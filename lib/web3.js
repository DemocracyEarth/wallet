...
const _generateDataset = async (feed, coin) => {
  ...
  for (let i = 0; i < feed.length; i += 1) {
    ...
    dataset.push({
      label: new Date(block.timestamp * 1000),
      data: parseFloat(_BNToNumber(feed[i].returnValues._value, coin), 10),
    });
  }
  return dataset;
};

const _buildChart = async (coin, publicAddress) => {
  const income = await _getTransfers(coin, publicAddress, 'Transfer', defaults.START_BLOCK, 'latest', { _to: publicAddress });
  const outcome = await _getTransfers(coin, publicAddress, 'Transfer', defaults.START_BLOCK, 'latest', { _from: publicAddress });

  console.log(income[0]);
  console.log(outcome[0]);

  const balance = await _generateDataset(income, coin);
  const dataset = Object.assign(balance, await _generateDataset(outcome, coin));
   // _.sortBy(Object.assign(await _generateDataset(income, coin), _.each(await _generateDataset(outcome, coin), (item) => { const newItem = item; newItem.data *= -1; return newItem; })), 'label');

/*

  for (let i = 0; i < income.length; i += 1) {
    balance.push({
      label: new Date(await _getEventBlock(income[i].blockNumber).timestamp * 1000),
      data: _BNToNumber(income[i].returnValues._value, coin),
    });
  }

  const dataset = _.sortBy(balance, 'label');
*/
      ...
      switch (collectives[i].profile.guild[k].name) {
        ...
        case 'guild-total-assets':
          ...
          if (coin) {
            ...
            const chart = await _buildChart(coin, daoGuild.publicAddress);
