...
const _getUserAddress = (res, collectiveId, role) => {
  ...
  for (let i = 0; i < addresses.length; i += 1) {
    ...
    // persist all collectives an address belongs to
    // const user = Meteor.users.findOne({ username: addresses[i].toLowerCase() });
    /* if (user && user.profile && user.profile.collectives && user.profile.collectives.length > 0) {
      settings.profile.collectives.push(...user.profile.collectives);
      settings.profile.collectives = _.uniq(settings.profile.collectives);
    } */
...
*/
const _checkPrecedent = (username, list, collectiveId) => {
  let shares = 0;

  for (let i = 0; i < list.length; i += 1) {
    if (list[i].username === username && list[i].collectiveId === collectiveId) {
...
const _mirrorTransactionState = (event, collectiveId, block) => {
  ...
  if (event.returnValues.didPass) {
    ...
    let reserves;
    const shares = parseInt(parseInt(new BigNumber(event.returnValues.sharesRequested).toNumber(), 10) + _checkPrecedent(authorUsername, precedentCache, collectiveId), 10);
...
const _mirrorCollectiveEvent = (event, collectiveId, block) => {
  ...
  const user = Meteor.users.findOne({ username: summoner });
...
/*
  console.log(summonerShare);

  if (user.profile.wallet.reserves.length > 0) {
    user.profile.wallet.reserves.push(summonerShare);
  } else {
    user.profile.wallet.reserves = [summonerShare];
  }

  console.log(user.profile.wallet.reserves);

  Meteor.users.update({ username: summoner }, { $set: { 'profile.wallet.reserves': user.profile.wallet.reserves } });
*/
...
const _mirrorUserEvent = (event, collectiveId, block) => {
  ...
  let reserves;
  const shares = parseInt(parseInt(_checkPrecedent(authorUsername, precedentCache, collectiveId) - new BigNumber(event.returnValues.sharesToBurn).toNumber(), 10), 10);
  ...
  if (member && member.profile.wallet && member.profile.wallet.reserves) {
