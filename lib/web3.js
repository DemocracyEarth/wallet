  ...
  if (user) {
    const blockDate = web3.eth.getBlock(event.blockNumber, (err, res) => {
      ...
      if (!err) {
        ...
        // elements.contractDescription = _getMatchingStateElement(state, elements, 'proposalQueue');
        console.log(elements);
        const contract = {
          title: parseContent(TAPi18n.__(map.contract.title), elements),
...
const _mirrorContractState = (state, index, collectiveId) => {
  ...
  log(`[web3] Mirroring blockchain proposal with...`);

  // create users required for this contract
  const authorUsername = _getAddressList(proposal, collectiveId);
  const user = Meteor.users.findOne({ username: authorUsername });
  ...
  if (user) {
    ...
    const contract = {
      ...
      closing: {
        ...
        height: 0, // parseInt(height + (votingTotalLength / blocktimes.ETHEREUM_SECONDS_PER_BLOCK), 10),
        calendar: new Date(proposal.startingPeriod * 1000),
        delta: 0, // parseInt((periodDuration * votingPeriodLength) / blocktimes.ETHEREUM_SECONDS_PER_BLOCK, 10),
      },
    };
...
const _getState = async (smartContract) => {
  ...
  if (smartContract.parameter) {
    ...
    for (let i = 0; i < smartContract.parameter.length; i += 1) {
      switch (smartContract.parameter[i].value) {
        case 'struct':
