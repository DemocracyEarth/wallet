...
const _buildChart = async (coin, publicAddress) => {
  ...
  const contract = await new web3.eth.Contract(erc20, coin.contractAddress).getPastEvents('Transfer', {
    filter: { src: publicAddress, dst: publicAddress },
    fromBlock: 9200000, // defaults.START_BLOCK,
    toBlock: 'latest',
  }, (error, res) => {
    if (error) {
      log('[web3] Error fetching log data.');
