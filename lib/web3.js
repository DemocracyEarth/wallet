...
const _generateDataset = async (feed, coin) => {
  ...
  for (let i = 0; i < feed.length; i += 1) {
    ...
    console.log(parseFloat(_BNToNumber(feed[i].returnValues._value, coin), 10));
    dataset.push({
        ...
        case 'guild-total-assets':
          ...
          if (coin) {
            ...
            const chart = [{
              ...
              type: 'lineal',
              dataset: [await _buildChart(coin, daoGuild.publicAddress)],
            }];

            log(`[dao] Total assets for this DAO... ${ticker} ${finalGuild[k].value}`);
            Collectives.update({ _id: collectives[i]._id }, { $set: { 'profile.guild': finalGuild, 'profile.chart': chart } });
          }
          break;
