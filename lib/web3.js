...
const _askOracle = async (coin) => {
  const abi = JSON.parse(coin.oracleABI);
  const contract = await new web3.eth.Contract(abi, coin.oracleAddress);
  let response;
  await contract.methods[coin.oracleMethod]().call({}, (err, res) => {
    if (err) {
      console.log(err);
    }
    response = res;
