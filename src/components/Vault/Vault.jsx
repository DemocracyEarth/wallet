...
const Vault = async (props) => {
  ...
  const vault = await new web3.eth.Contract(ubidaiABI, this.props.address);

  const depositLimit = '50000000000000000000000'
    ...
    <div className="vote vote-search vote-feed nondraggable vote-poll">
      <div className="checkbox checkbox-custom">
        <div className="meta meta-search meta-bar">
          ...
          <div className="dao">
            <Account publicAddress={props.address} icon={contract} format="icon" width="16px" height="16px" href={`${config.web.explorer.replace('{{publicAddress}}', props.address)}`} />
          ...
          <div className="option-title option-link option-search title-input">
            <div className="title-input title-feed">
              <div className="title-header">
                {typeof props.title === 'string' ? parser(props.title) : props.title}
              </div>
              {
                (props.description) ?
                  <div className="title-description">
                    {typeof props.description === 'string' ? parser(props.description) : props.description}
                  </div>
                  :
                  null
              }
              {
                (props.link) ?
                  <div className="title-description">
                    <a href={props.link} target="_blank" rel="noopener noreferrer" onClick={(e) => { e.stopPropagation(); }}>{props.link}</a>
                  </div>
                  :
                  null
              }
          ...
          >
            <Contract hidden={false} view={routerView.PROPOSAL} href={'https://etherscan.io/address/0x8EBd041213218953109724e60c9cE91B57887288'}>
              <Parameter label={i18n.t('deposit-limit')}>
                <Token quantity={depositLimit} publicAddress={'0x6b175474e89094c44da98b954eedeac495271d0f'} symbol={'DAI'} decimals={18} />
              </Parameter>
